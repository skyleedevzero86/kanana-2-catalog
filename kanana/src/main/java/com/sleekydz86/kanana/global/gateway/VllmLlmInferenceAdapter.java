...
public class VllmLlmInferenceAdapter implements LlmInferencePort {
    ...
    public String complete(String modelId, String message) {
        ...
        try {
            response = webClient.post()
                    ...
                    .bodyValue(body)
                    .retrieve()
                    .onStatus(HttpStatusCode::is4xxClientError, r -> Mono.just(new LlmInferenceException("LLM 서버 요청이 거부되었습니다. (HTTP " + r.statusCode().value() + ")")))
                    .onStatus(HttpStatusCode::is5xxServerError, r -> Mono.just(new LlmInferenceException("LLM 서버 오류가 발생했습니다. 잠시 후 다시 시도해 주세요. (HTTP " + r.statusCode().value() + ")")))
        ...
        if (response == null || response.getChoices() == null || response.getChoices().isEmpty()) {
            log.warn("vLLM 응답에 결과가 없습니다. 모델: {}", effectiveModel);
            return "";
        }
    ...
    public static class VllmChatResponse {
